...
def run_process(queue, results_queue, semaphore, main_logger, row_id, lamb, parts):
    with semaphore:
        ...
        try:
            ...
            logger = setup_logger(cfg, lamb, proc_id)
            ...
            _, finished = pyaccord_sp(torch.from_numpy(X).type(flt).to(device), lamb, cfg, logger, part = parts[row_id], omega_old = omega_old, label = row_id + cfg["label_start"], device = device)
    ...
    if cfg["row_divide"] == 0:
        ...
        for lamb in lambs:
            ...
            for i in range(cfg["row_divide"]):
                p = mp.Process(target=run_process, args = (proc_queue, results_queue, semaphore, main_logger, i, lamb, parts))
                p.start()
                processes.append(p)
